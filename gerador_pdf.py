...
def gerar_pdf_bytes(df_resultados, total_linhas):
    ...
    pdf.multi_cell(0, 6, f"O scanner analisou com sucesso um total de {total_linhas} linhas. Com base nas regras automatizadas de identificação (Regex e Heurística), foi gerado o inventário de dados pessoais descritos a seguir.")
    pdf.ln(5)
    
    # Seção 2: Inventário Encontrado e Nível de Risco
    pdf.set_font("Arial", "B", 12)
    pdf.set_text_color(43, 108, 176) 
    pdf.cell(0, 10, "2. Inventário de Dados Pessoais Detectados", ln=True)
    pdf.ln(2)
    
    # Cabeçalho da Tabela (Redistribuição dos 190 pontos)
    pdf.set_font("Arial", "B", 10)
    pdf.set_fill_color(226, 232, 240) 
    pdf.cell(60, 8, "Coluna do Arquivo", border=1, fill=True)
